...
class gainXPWindow extends Application {
	...
	async applyXP() {
		...
		let targetvalue = Number(this.actor.system.xp.value) + gainedXP;
...
class spendXPWindow extends Application {
	constructor(actor, Options = {}) {
		...
		this.xpavailable = this.actor.system.xp.value;
		if (this.xpavailable == null) {
			this.xpavailable = 0;
		}

		const attributes = {...this.actor.system.attributes};
		let skillitems = this.actor.items.filter(item => item.type == "skill");
		
		//sort the skills (why are they not sorted alread?)
		let skillgroups = {};
		
		for (let attribute of Object.keys(attributes)) {
			skillgroups[attribute] = skillitems.filter(skill => skill.system.attribute == attribute);
		...
		skillitems = [];
		Object.values(skillgroups).forEach(group => skillitems = skillitems.concat(group));
		
		let skills = {};
		let skillincreases = {};
		
		for (let skillitem of skillitems) {
			skills[skillitem.system.skillKey] = Number(skillitem.system.value);
			skillincreases[skillitem.system.skillKey] = 0;
		}
		...
		this.availableTalents = this.availableTalents.filter(talent => talent.system.creatureType == this.actor.system.creatureType);
		this.chosenTalents = [];
	...
	async getHTML(pOptions={}) {
		...
		for (const key of Object.keys(skills)) {
			entriesHTML = entriesHTML + 	`<div class="skillchoice" name="${key}" style="display:flex;margin-top:3px;margin-bottom:3px">
												<label style="width:50%">${game.i18n.localize("MYZ.SKILL_" + key)}</label>
	...
	async applyChoices() {
		if (await this.calculateXPleft() >= 0) {
			...
			//skills
			for (let key of Object.keys(this.skillincreases)) {
				this.actor.items.find(skill => skill.system.skillKey == key)?.update({system : {value : this.originalSkills[key] + this.skillincreases[key]}})
			}
			
			//XP
			this.actor.update({system : {xp : {value : this.actor.system.xp.value - (await this.calculateXPused())}}});
		}
	}
...
function xpThreshholdReached(actor) {
	return actor.system.xp.value >= Math.min(skillcost, talentcost);
}
